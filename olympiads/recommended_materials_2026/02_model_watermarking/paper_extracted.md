...
search articles without training. For example, Model Zoo 1
...
tems. In the future, more systematic model-sharing plat-
...
skills (e.g. Alexa Skills 2) may appear, similar to Google
Play or App Store. In these situations, it is necessary to
...
1
arXiv:1701.04082v2  [cs.CV]  20 Apr 2017
...
bedT -bit vectorb∈{ 0, 1}T into the parameters of one or
...
Capacity The effective watermarking system must have the ability to embed a large amount of in-
formation.
Security A watermark should in general be secret and should not be accessed, read, or modiﬁed by
unauthorized parties.
Efﬁciency The watermark embedding and extraction processes should be fast.
...
into one of the convolutional layers in a host DCNN 3. Let
...
W . The tensor W is a set ofL convolutional ﬁlters and the
...
der of ﬁlters, we calculate the mean of W overL ﬁlters as
W ijk = 1
L
∑
lWijkl. Letting w∈ RM (M =S×S×D)
denote a ﬂattened version ofW , our objective is now to em-
bedT -bit vectorb intow.
...
embedding a watermark whiletraining a host network for
...
the original task. The cost functionE(w) with a regularizer
is deﬁned as:
E(w) =E0(w) +λER(w), (1)
whereE0(w) is the original cost function,ER(w) is a regu-
larization term that imposes a certain restriction on param-
etersw, and λ is an adjustable parameter. A regularizer is
...
fromw. Given a (mean) parameter vector w∈ RM and an
embedding parameter X∈ RT×M , the watermark extrac-
tion is simply done by projecting w usingX, followed by
thresholding at 0. More precisely, the j-th bit is extracted
as:
bj =s(ΣiXjiwi), (2)
wheres(x) is a step function:
s(x) =
{
1 x≥ 0
0 else . (3)
...
whereyj =σ(ΣiXjiwi) andσ(·) is the sigmoid function:
σ(x) = 1
1 + exp(−x). (5)
We call this loss function an embedding loss function. It
may be confusing that an embedding loss function is used
to updatew, notX, in our framework. In a standard percep-
tron,w is an input andX is a parameter to be learned. In our
case,w is an embedding target and X is a ﬁxed parameter.
The design ofX is discussed in the following section.
...
needs to guide model parameters to one of a number ofgood
...
in both embedding and extraction of watermarks. In this
...
j-th bitbj is embedded into the difference betweenwi+ and
wi− whereX diff
ji+ = 1 andX diff
ji− =−1.
...
CIFAR-10 and Caltech-101 datasets. The CIFAR-10
...
convolution layers: the second convolution layer in the
...
berM of parameter w is also shown for these layers. The
parameterλ in Eq. (1) is set to 0.01. As a watermark, we
embeddedb = 1∈{ 0, 1}T in the following experiments.
...
Group Output size Building block #w
...
and dashed lines denote training lossE(w) (y-axis on the right).
Table 4. Test error (%) and embedding lossER(w) with and with-
out embedding.
Test error ER(w)
...
embedded case if the parametersX direct andX diff are used.
...
two parameters. On the other hand, the training loss ofEm-
...
rameters are used in embedding and detection. If we bi-
...
that we embeddedb = 1∈{ 0, 1}T as a watermark as men-
...
ofσ(ΣiXjiwi) is quite different from those for embedded
cases.
...
300Frequency
Not embedded
Embedded (random) (c) random
Figure 2. Histogram of the embedded watermarkσ(ΣiXjiwi) (before thresholding) with and without watermarks.
6
 4
 2
 0 2 4 6
...
0.14Frequency
value (d) random
Figure 3. Distribution of model parametersW with and without watermarks.
...
ﬁne-tune-to-embed, two experiments were performed. In
...
bedded achieved almost the same test error asNot embed-
ded 2nd and a very lowER(w).
...
achieved almost the same test error asNot embedded 2nd
...
of the second training, while the learning rate was reduced to8.0 × 10−4)
at the end of the ﬁrst training.
6

Table 5. Test error (%) and embedding lossER(w) with and with-
...
10 dataset with and without embedding. In this second
...
proposed method also achieved low test error andER(w) in
...
izer.
Table 6. Test error (%) and embedding lossER(w) for the combi-
...
ding a watermark with differentλ.
λ 1
2||w−w0||2
2 ER(w) Test error BER
...
ER(w) E′
R(w) Test error
CIFAR-10→ CIFAR-10 4.76×10−4 8.66×10−4 7.69
Caltech-101→ CIFAR-10 5.96×10−3 1.56×10−2 7.88
...
dataset. We removedα% of the 3× 3× 64× 64 parameters
of the embedded layer and calculated embedding loss and
bit error rate. Figure 5 (a) shows embedding loss ER(w)
as a function of pruning rateα. Ascending (Descending)
represents embedding loss when the topα% parameters are
...
shows the histogram of the detected watermarkσ(ΣiXjiwi)
after pruning forα = 0.8 and 0.95. For α = 0.95, the his-
...
nal watermark. In our preliminary experiments, this wa-
...
Embedded α = 0. 8
Embedded α = 0. 95
Figure 6. Histogram of the detected watermarkσ(ΣiXjiwi) after
pruning.
...
Theano: a CPU and GPU math expression compiler. InProc.
...
Y . LeCun. The loss surfaces of multilayer networks. InProc.
of AISTATS, 2015.
...
in a neural network. In Proc. of NIPS Workshop on Deep
...
architecture for fast feature embedding. In Proc. of MM ,
2014.
...
networks for large-scale image recognition. InProc. of ICLR,
2015.
