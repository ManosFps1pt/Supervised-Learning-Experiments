...
- `olympiads/` is the active preparation area for IOAI 2026 first, with CEOAI / EUROAI material kept as useful regional practice and history.
...
4. Compare new material against the official IOAI syllabus first, then against CEOAI / EUROAI material when relevant.
...
When the user is close to IOAI/CEOAI or explicitly says preparation must happen
in a three-day sprint, switch from general learning to exam-maximizing coaching.

In this mode:

- Direct IOAI syllabus coverage is the main metric until IOAI 2026 is over.
- Read `olympiads/priority.md` first when deciding what the user should study
  next. This file is generated by the scheduled IOAI coach and is the current
  cross-chat handoff.
- Calendar rule for IOAI 2026: the last real study day is 2026-07-30. The user
  cannot study on 2026-07-31, and flies to Astana on 2026-08-01, so those days
  are rest, logistics, packing, and travel only.
...
- For contest-environment practice, use JupyterLab as the main interface. VS
  Code may be used as a secondary offline editor, but mock contests should not
  rely on AI coding assistants, copilots, external LLMs, unrestricted web
  browsing, or external APIs.
...
- IOAI 2026 schedule: https://ioai-official.org/republic-of-kazakhstan/schedule/
